...
public class DestinationValidator extends Validator {
    ...
    public ErrorResponse addNewDestination() {
        ...
        if (this.required("name") && this.isText("name")) {
            this.minTextLength("name", 1);
        }

        // Checks destination has an owner with an ID
        if (this.required("user") && this.form.get("user").get("id").asText("").equals("")) {
            this.required("userId");    // Yes I'm a bad person
        }

        // Check that the destination type is present, is text, and not empty
        if (this.required(TYPE) && this.isText(TYPE)) {
