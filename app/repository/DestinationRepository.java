...
public class DestinationRepository {
     ...
     */
    public CompletableFuture<Result> makeDestinationPublic(User user, Destination destination, Long masterAdminId) {
        return supplyAsync(() -> {
            ...
            if (rowsChanged > 0) {
                makePermanentlyPublic(destination, masterAdminId);
            }
            return ok();
        });
    }
     ...
     */
    public CompletableFuture<Destination> checkDestinationInTrip(Destination destination, Long userId, Long masterAdminId) {
        return supplyAsync(() -> ebeanServer.find(Destination.class)
                        .where()
                        .idEq(destination.id)
                        .ne("user_id", userId)
     ...
     */
    public CompletableFuture<Destination> makePermanentlyPublic(Destination destination, Long masterAdminId) {
        // Change ownership to master admin and ensures destination is public
        destination.user.id = masterAdminId;
        destination.isPublic = true;
        return supplyAsync(() -> {
            ebeanServer.update(destination);
            return destination;
