Feature: News Feed Events
  ...
  Scenario: New cover photo
    ...
    When I get the news feed events for profile 1
    Then There will be 1 news feed event
    And The first news feed event will have type 'NEW_PROFILE_COVER_PHOTO'
  ...
  Scenario: New public photo, then cover photo
    ...
    And I set it as my cover photo
    When I get the news feed events for profile 1
    Then There will be 2 news feed event
    And The first news feed event will have type 'NEW_PROFILE_COVER_PHOTO'
    And The news feed event at index 1 will have type 'MULTIPLE_GALLERY_PHOTOS'
  ...
  Scenario: Created private destination, then set to public
    ...
    Then There will be 1 news feed event
    And The first news feed event will have type 'UPDATED_EXISTING_DESTINATION'
  ...
  Scenario: Created public destination
    ...
    When I get the news feed events for profile 2
    Then There will be 1 news feed event
    And The first news feed event will have type 'CREATED_NEW_DESTINATION'